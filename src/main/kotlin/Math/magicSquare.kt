...
fun numMagicSquaresInside(grid: Array<IntArray>): Int {
        ...
        fun isMagicSquare(grid: Array<IntArray>): Boolean {
            val sum = grid[0].sum()
            ...
            for (i in grid.indices) {
                ...
                if(grid[i].max() > 9) return false
                if(grid[i].min() < 1) return false
        ...
        for(i in grid.indices){
            for(j in grid[i].indices){
                if(i + 2 < grid.size){
                    if(j + 2 < grid[i].size){
                        var tempGrid = grid.sliceArray(IntRange(i, i+2))
                        for(row in tempGrid.indices){
                            tempGrid[row] = tempGrid[row].sliceArray(IntRange(j, j+2))
                        }
